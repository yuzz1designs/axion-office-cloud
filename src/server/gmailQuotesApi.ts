...
export async function handleGmailQuotesApi(req: IncomingMessage, res: ServerResponse, next: () => void) {
  ...
  try {
    ...
    if (url.pathname === "/api/quotes/gmail/status" && req.method === "GET") return json(res, 200, { configured: Boolean(config()), connected: Boolean(current), shared: true, scope: "workspace", email: current?.provider_email, expectedEmail: expectedEmail(), lastSyncAt: current?.last_sync_at, error: current?.last_error });
    if (url.pathname === "/api/quotes/gmail/oauth/start" && req.method === "GET") { if (current) return redirect(res, "/?quotes-gmail=connected"); const oauthConfig = config(); if (!oauthConfig) return json(res, 503, { error: "Configura o OAuth e a chave de encriptação no servidor." }); return redirect(res, authUrl({ clientId: oauthConfig.clientId, redirectUri: oauthConfig.redirectUri, state: await states.create(context.user.id) })); }
    ...
    if (url.pathname === "/api/quotes/gmail/sync" && req.method === "POST") {
      ...
      } catch (error) { const message = error instanceof Error ? error.message : "GMAIL_SYNC_FAILED"; await context.backend.client.from("workspace_integrations").update({ status: "connected", last_error: message, updated_at: new Date().toISOString() }).eq("workspace_id", context.workspaceId).eq("provider", provider); throw error; }
    }
