...
export function subscribeAxionRealtime(config: SupabasePublicConfig, onChange: (table: string) => void) {
  ...
  const channel = supabase.channel("axion-office")
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "quote_requests" }, () => onChange("quote_requests"))
    .on("postgres_changes", { event: "*", schema: "public", table: "quotes" }, () => onChange("quotes"))
    .on("postgres_changes", { event: "*", schema: "public", table: "quote_services" }, () => onChange("quote_services"))
